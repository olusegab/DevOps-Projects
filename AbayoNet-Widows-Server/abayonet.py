    ...
    for attempt in range(8):
        try:
            ...
            if code in _RETRYABLE or 'lock wait timeout' in str(e).lower() or 'deadlock' in str(e).lower():
                ...
                # e.g. connection lost mid-transaction — drop the cached
                # connection so the next call reconnects instead of reusing
                # a broken one, then retry once.
                close_thread_db()
                if attempt < 7:
                    time.sleep(0.5)
                else:
...
def init_db():
    raw = pymysql.connect(
        ...
        user=_DB_CONF['user'], password=_DB_CONF['password'],
        database=_DB_CONF['database'], charset='utf8mb4',
        cursorclass=RowDictCursor, autocommit=False,
    )
    c = raw.cursor()
    ...
    def do_POST(self):
        ...
        try:
            ...
            if '/toggle' in path and path.startswith('/api/hosts/'):
                ...
                db_exec('UPDATE hosts SET enabled=? WHERE id=?',(enabled,hid))
            ...
            if path.startswith('/api/hosts/') and '/toggle' not in path and '/bulk' not in path and path!='/api/hosts/import':
                ...
                try:
                    db_exec('UPDATE hosts SET ip=?,name=?,group_name=?,tags=?,description=?,location=?,alert_email=?,ping_interval=?,packet_count=?,timeout_ms=?,port_checks=?,notes=? WHERE id=?',
